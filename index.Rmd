...
ggplot(data = enroll_map, aes(y = log(log(percentdegree + 1)), x = out_of_state_tuition)) +
  geom_point(aes(color = institution.x)) + theme(legend.position = "none") + ggtitle("Transformed percent Sustainability Studies completions\ncompared to out of state tuition in ten thousands of dollars") + xlab("Out of State Tuition (ten thousands of dollars)") + ylab ("Transformed % Sustainability Studies degrees (log-log transformed)")
...
anim_plot <- ggplot(enroll_map, aes(log(log(percentdegree + 1)))) + geom_density(col = "black",fill = "blue") + ggtitle("{frame_time}") + xlab("Transformed percent degree completions(log-log transformed)") + ylab("frequency") + transition_time(year) + shadow_mark(alpha = .3) + ease_aes("linear") +
  enter_fade() +
  exit_fade()
